...
def test_file_info():
    ...
    assert file_info.object == "file"
    assert file_info.bytes == 1000
    assert file_info.created_at == 1623456789
    assert file_info.filename == "test.jsonl"
    assert file_info.purpose == "fine-tune"
    assert file_info.hash == "abc123"
    
    dict_data = file_info.to_dict()
...
def test_job_info():
    ...
    assert job_info.object == "fine_tuning.job"
    assert job_info.model == "gpt-3.5-turbo"
    assert job_info.created_at == 1623456789
    assert job_info.finished_at == 1623457789
    assert job_info.fine_tuned_model == "ft:gpt-3.5-turbo:org:custom_suffix:id"
    
    dict_data = job_info.to_dict()
    assert dict_data == data
